...
begin
  result := True;
  if TryStrToDate(editGeburtsdatum.Text, aDate) or (trim(editGeburtsdatum.Text) = '') then
  begin
    TARC_DAL_Person.updatePerson(updatePerson, editVorname.Text, editNachname.Text, editLand.Text, editBundesland.Text,
      comboBogenkategorie.Text, comboAlterskategorie.Text, comboGeschlecht.Text, aDate,
      TARC_DAL_DbUpdate.getVereinID(queryPerson.connection, comboVerein.Text), FPE_ID, checkLizenz.Checked,
      checkLandeswertung.Checked, trim(editGeburtsdatum.Text) <> '');
